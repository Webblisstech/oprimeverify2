...
class HomeController extends Controller
{
    ...
    public function order_now(Request $request)
    {
        ...
        if ($order == 0) {
            User::where('id', Auth::id())->increment('wallet', $request->price);
            $message = "OPRIMEVERIFY | Low balance";
    ...
    public function fund_now(Request $request)
    {
        ...
        if ($request->type == 1) {
            ...
            $message = Auth::user()->email . "| wants to fund |  NGN " . number_format($request->amount) . " | with ref | $ref |  on OPRIMEVERIFY";
        ...
        if ($request->type == 2) {
            ...
            $message = Auth::user()->email . "| wants to fund Manually |  NGN " . number_format($request->amount) . " | with ref | $ref |  on OPRIMEVERIFY";
            send_notification2($message);
    ...
    public function fund_manual_now(Request $request)
    {
        ...
        $message = Auth::user()->email . "| submitted payment receipt |  NGN " . number_format($request->amount) . " | on OPRIMEVERIFY";
        send_notification2($message);
    ...
    public function verify_payment(request $request)
    {
        ...
        if ($status == 'failed') {
            ...
            $message = Auth::user()->email . "| Cancled |  NGN " . number_format($request->amount) . " | with ref | $trx_id |  on OPRIMEVERIFY";
            send_notification2($message);
        ...
        if ($trxstatus == 2) {
            ...
            $message =  Auth::user()->email . "| on OPRIMEVERIFY | is trying to fund  with | " . number_format($request->amount, 2) . "\n\n IP ====> " . $request->ip();
            send_notification2($message);
        ...
        if ($status1 == 'success') {
            ...
            $message = Auth::user()->email . "| Just funded |  NGN " . number_format($request->amount) . " | with ref | $order_id |  on OPRIMEVERIFY";
            send_notification2($message);
    ...
    public function session_resolve(request $request)
    {
        ...
        if ($trx == null) {
            ...
            $message = Auth::user()->email . "is trying to resolve from deleted transaction on OPRIMEVERIFY";
            send_notification($message);

            $message = Auth::user()->email . "is trying to reslove from deleted transaction on OPRIMEVERIFY";
            send_notification2($message);
        ...
        if ($chk == 2 || $chk == 4) {
            ...
            $message = Auth::user()->email . "is trying to steal hits the endpoint twice on OPRIMEVERIFY";
            send_notification($message);
        ...
        if ($status == 'true') {
            ...
            $message = Auth::user()->email . "| just resolved with $request->session_id | NGN " . number_format($amount) . " on OPRIMEVERIFY";
            send_notification($message);
    ...
    public function reset_password(request $request)
    {
        ...
        if ($ck == $request->email) {
            ...
            $data = array(
                'fromsender' => 'info@oprimeverify.com', 'OPRIMEVERIFY',
    ...
    public function  resolveNow(request $request)
    {
        ...
        if ($ck_trx == 2) {
            ...
            $email = Auth::user()->email;
            $message =  "$email | OPRIMEVERIFY | is trying to fund and a successful order with orderid $request->trx_ref";
            send_notification2($message);
        ...
        if ($ck_trx == 1) {
            ...
            if ($status == true) {
                ...
                $message = "$user_email | $request->trx_ref | $session_id | $var->amount | just resolved deposit | OPRIMEVERIFY";
                send_notification($message);
                send_notification2($message);
